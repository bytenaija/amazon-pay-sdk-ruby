...
module AmazonPay
  class AmazonPayClient
    ...
    def api_call(options)
      ...
      prepared_options[:headers] = AmazonPay::ClientHelper.sign_headers(@config_args, prepared_options)
      AmazonPay::ClientHelper.invoke_api(@config_args, prepared_options)
    end
    ...
    def get_signed_headers(options)
      prepared_options = AmazonPay::ClientHelper.prepare_options(@config_args, options)
      AmazonPay::ClientHelper.signHeaders(@config_args, prepared_options)
    end
    ...
    def get_authorization_token(mws_auth_token, merchant_id, headers = null)
      api_call({
                  method: 'GET',
    ...
    def generate_button_signature(payload)
      AmazonPay::ClientHelper.sign_payload(@config_args, payload)
    end
    ...
    def delivery_trackers(payload, headers = null)
      api_call({
                method: 'POST',
